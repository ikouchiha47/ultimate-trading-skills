...
def main():
    ...
    if not table.empty:
        print("\nRank  Symbol         CAGR    MaxDD   Calmar  Ret(win)  ExcessVsSec  AvgDeliv%  Up  25dmaZ")
        for sym, row in table.iterrows():
            ...
            print(f"{int(row['rank']):>3}  {sym:14s} "
                  ...
                  f"  {up:>2}  {_num(row.get('dist_25dma_z')):>6}")
        print("(25dmaZ << 0 = unusually stretched below its 25-DMA — the BNF fade candidate, manuals/03)")
    if events:
